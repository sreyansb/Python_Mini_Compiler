...
CODE LOGIC: 
O(n) code
Loop Invariancy can be applied to loops only(duh)
	According to what we have developed IF(operator) signifies a loop with the result as the label to go to if the condition is true
It is a recursive code
...
fptr=open("optimized/showfopr.tsv","r")
...
f=open("optimized/showinvariant.tsv","w")
