...
for_stmt
	: T_For T_ID T_IN range_stmt T_Cln suite optional_else {insertRecord("Identifier", $<text>2, @2.first_line, currentScope+1); 
				char rangeNodeText[20] ="";
				strcat(rangeNodeText, $<text>2);
				strcat(rangeNodeText, " in range");
				clearArgsList(); }
	| T_For T_ID T_IN T_ID T_Cln suite optional_else {insertRecord("Identifier", $<text>2, @2.first_line, currentScope+1); 
		 //printSTable();
...
/*
suite : simple_stmt
		| T_NL ID {initNewTable($<depth>2); updateCScope($<depth>2);} finalStatements a_suite;

a_suite : T_NL ND finalStatements a_suite 
		| T_NL end_suite ;
